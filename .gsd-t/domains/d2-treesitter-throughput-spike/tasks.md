...
- **Test**: `test/m94-k2-treesitter-throughput.test.js` — fixture-based (no Atos repo needed); asserts verdict logic at/around the ~2-min budget threshold (a synthetic over-budget run → KILL), the `repo-not-found` FAIL-LOUD path, **[#7 Atos pin]** the `commit-mismatch`/unpinned-repo FAIL-LOUD path (a recorded wall-clock with no pinned SHA is rejected), and the envelope shape (which MUST carry the `atosSha` field); **[RE-PLAN Fix-6 — measured scale]** asserts the envelope carries `atosFileCount` + `atosTotalLoc` + `atosLangBreakdown` (scale MEASURED, not the ~1.5M assumption) and that a synthetic `scaleMismatch` run (measured scale materially diverging from the D1 synthetic bake-off scale) flags `scaleMismatch:true` and FAILS the gate (`[RULE] k2-atos-scale-measured-not-assumed` · `[RULE] k2-scale-sanity-vs-bakeoff`). The real-repo wall-clock + measured scale are runtime spike measurements recorded in T3's result doc, NOT test assertions.
...
- **Acceptance criteria**:
  ...
  - **[RE-PLAN Fix-6 — measure real Atos scale, never assume ~1.5M]** at the pinned SHA, the probe MEASURES and records the REAL Atos scale into the envelope: `atosFileCount` (source files enumerated by the parse harness), `atosTotalLoc`, and `atosLangBreakdown` (per-language LOC/file split via `scc`/`cloc` or the parse harness's own enumeration). The `~1.5M LOC` figure is NOT asserted as known — it is the hypothesis the measurement confirms or refutes. `[RULE] k2-atos-scale-measured-not-assumed` — the envelope MUST carry these three fields. The probe ALSO emits `scaleDivergenceVsBakeoff`: the measured Atos scale compared against the D1 synthetic-generator scale (the bake-off's assumed ~1.5M nodes); if the measured scale diverges MATERIALLY (e.g. > 1.5× or < 0.66×) from the synthetic scale, the envelope flags `scaleMismatch: true` and the K2 verdict is FAIL-LOUD (K1's store evidence was validated at the WRONG size — `[RULE] k2-scale-sanity-vs-bakeoff`). The killing test asserts a synthetic scale-mismatch run flags `scaleMismatch` and fails the gate.
...
- **Acceptance criteria**:
  - Records the real-repo build wall-clock + PASS/KILL verdict + the pinned Atos commit SHA + **[Fix-6 footprint]** the measured peak build RSS against the pre-registered ceiling with a LIVE-CLOCK timestamp
  - **[RE-PLAN Fix-6 — measured scale]** records the MEASURED Atos scale (`atosFileCount` + `atosTotalLoc` + `atosLangBreakdown`) + the `scaleDivergenceVsBakeoff` comparison against the D1 synthetic scale; if `scaleMismatch:true`, the verdict is FAIL-LOUD and the doc records that K1's store evidence must be re-validated at the corrected scale before any Wave-2 task (`[RULE] k2-scale-sanity-vs-bakeoff`)
  - **[RE-PLAN Fix-4 — machine-checkable Wave-1 gate]** the result-doc envelope MUST carry a structured `k2Verdict` field with value `PASS` or `KILL` (the verdict the Wave-1 hard-gate test reads) — `[RULE] k2-verdict-field-machine-checkable`. The integrate-owned Wave-1 hard-gate test (D7-T2) reads this field; an envelope missing `k2Verdict` FAILS the gate
  - progress.md updated with the build wall-clock (AC-1) + **[Fix-6]** the peak build RSS + the MEASURED Atos scale + the pinned SHA
