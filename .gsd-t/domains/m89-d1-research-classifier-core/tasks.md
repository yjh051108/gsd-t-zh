...
#### Held-out generalization fixture (finding #1 — paired with T1)
**Files**: `test/fixtures/m89-heldout-corpus.json` (NET-NEW — already authored at plan time)
**Description**: ≥6 NOVEL gaps NOT used to author/tune the classifier — the anti-self-fulfilling-oracle guard. 3 external (Stripe webhook signature-verification header spec; Chrome `storage.local` per-origin storage-quota limit; CSS `:has()` browser-support/version) + 3 internal (`isOrderLocked` return in this repo; which domain owns `templates/workflows/gsd-t-verify.workflow.js`; what exit code `cli-preflight` uses on a wrong branch). Each item carries `{ id, source, gap, expectedClass, expectedRoute, featureSignal }`; the `featureSignal` field NAMES the feature class that drives the label (proof it's a feature decision, not a keyword match). **None of these proper nouns/symbols appears anywhere in `m89-labeled-corpus.json`** — so a classifier that keyword-matched the seen 13 cannot pass these.
**Acceptance**: valid JSON; ≥6 items (≥3 external, ≥3 internal); zero token overlap with the seen-corpus proper nouns (`PayPal`/`OAuth`/`invoice`); every item single-labeled.
**Test**: consumed by T3's HELD-OUT assertion set; a classifier passing the seen 13 but failing any held-out item FAILS A1.
...
- **GENERALIZATION REQUIREMENT (finding #1 — the anti-self-fulfilling-oracle guard):** the classifier MUST decide by **feature class**, not by hard-matching corpus strings. A router that hard-matches the literal tokens `PayPal`/`OAuth`/`invoice` scraped from the 13 gap strings would pass the seen corpus 13/13 yet generalize to NOTHING — the shallow-test trap. The discriminator is: **does the gap reference an EXTERNAL signal** (a recognized third-party / library / platform / spec **proper noun**; an API / endpoint / HTTP / OAuth / webhook / spec / version / limit / rate-limit / error-shape / auth-flow **term**; a browser / runtime behavior) **vs. an INTERNAL signal** (a repo-relative **path**, a known **local symbol/file**, an explicit "this repo / our / the existing" **anchor**). Implement the heuristic as feature/signal-class detection (contract §1), NOT a per-corpus allowlist.
...
- **HELD-OUT (finding #1 — the generalization / anti-self-fulfilling-oracle assertion):** Load `test/fixtures/m89-heldout-corpus.json` (≥6 NOVEL gaps NOT used to author the classifier: 3 external — Stripe webhook signature-header spec; Chrome storage-quota limit; CSS `:has()` browser-support — + 3 internal — `isOrderLocked` return in this repo; which domain owns `gsd-t-verify.workflow.js`; what exit code `cli-preflight` uses). Assert the classifier labels EACH held-out item correctly by feature class. **"Passes the seen 13 but FAILS any held-out item" is an EXPLICIT FAILURE** — it proves the classifier memorized corpus keywords instead of generalizing by feature class. None of the held-out proper nouns/symbols appears in the seen corpus, so a keyword-scraping router cannot pass this set.
