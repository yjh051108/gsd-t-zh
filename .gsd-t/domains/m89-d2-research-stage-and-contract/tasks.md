...
**CONTRACT v1.1.0 (plan-phase pre-mortem fixes — findings #1/#2/#3/#5):** the contract is bumped v1.0.0 → **v1.1.0** with a Changelog entry covering:
- **§1.1 (finding #1):** classification is by FEATURE CLASS (external-signal vs. internal-signal sets), NOT corpus-keyword matching; the deterministic signal-class heuristic is defined precisely; A1 adds a HELD-OUT corpus so a keyword-memorized classifier FAILS.
- **§4.1 (finding #2):** "covers" = exact normalized-gap-key match (NOT substring/keyword/fuzzy), with the PayPal-OAuth-vs-invoice-TOTAL negative example.
- **§5.1 (finding #3):** the ambiguous → internal-first → grep → escalate-to-external capability is owned + functionally tested by the WIRING domains (D3/D4), not orphaned/deferred.
- **§5/A3 (finding #5):** A3 reconciled to the routing-decision wording + structurally enforced (research stage is the SOLE web-tool-granting `agent()`).
**Acceptance**: §2 names the bare-`fable`-literal form, explicitly rejects the `??`-research form with the lint rationale; §1.1 feature-class heuristic, §4.1 exact-gap-key "covers", §5.1 escalation ownership, and §5/A3 routing-decision wording are present; the Changelog records v1.1.0 covering findings #1/#2/#3/#5; the §2 model-form correction is documented (clarification of the intended form). D1/D3/D4 build against §1.1/§3/§4.1/§5/§6.
