...
## 49. Wire the Architect (Six-Stage Pass) into Scan — findings → simplest-fix designs
- **Type:** architecture | **App:** gsd-t | **Category:** workflow-integration
- **Added:** 2026-07-12 | **Priority:** MEDIUM | **Origin:** conversation 2026-07-12 (right after M101 `/gsd-t-architect` shipped v5.0.11)
- **Problem:** Scan is a DETECTOR ("what's wrong?" — dead code, duplication, risks, coupling, and the **Consolidation Opportunities** clusters it already produces). The Architect (M101) is a DESIGNER ("what's the simplest correct fix given what already exists?"). Today scan hands you a tech-debt register and STOPS — it never turns "here's the debt" into "here's the simplest fix." The two are complementary, not the same: do NOT merge them (merging would blur scan's exhaustive breadth and the architect's focused depth). The missing link is a HANDOFF: scan's highest-value findings — especially the "got-complicated-over-time" accretions and duplication families (which are literally the architect's Stage 3/5 reuse+duplication input already teed up) — should feed the architect's Six-Stage Pass.
- **This item — do BOTH halves (user directive 2026-07-12):**
  1. **Future scans (automated follow-on).** Add an OPT-IN final phase to the scan workflow: after the debt register + Consolidation Opportunities are produced, run the Architect's Six-Stage Pass on the top-N clusters/accretions → emit a simplest-fix design as plain-English pseudocode per cluster (house style). Scan stays pure (detector); the architect phase is additive and opt-in, so it never bloats a plain "just find the debt" run. So future scans can AUTOMATE the find→design step.
  2. **Past scans (retroactive).** `/gsd-t-architect` must be able to take an EXISTING scan register as its `$TARGET` — read `.gsd-t/scan/` (or the techdebt register) and run the pass on already-produced findings, so a scan done last week can be upgraded to designs today WITHOUT re-scanning. (The command already accepts a description/path target; this is a concrete "read the scan register" input mode + doc.)
