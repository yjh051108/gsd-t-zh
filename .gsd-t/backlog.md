...
- **Pairs with #48** (logging) — both are "framework-default infrastructure baked into every project" and could ship as one milestone or two.

## 48. Universal Trace + Audit Logging (contract + rule + scaffolding)
- **Type:** architecture | **App:** gsd-t | **Category:** contracts
- **Added:** 2026-07-01 | **Priority:** HIGH (promote-later) | **Origin:** BinVoice + WindowsVoiceTranscription debugging struggles
- **Problem:** Repeatedly hard/impossible for GSD-T to debug issues that would be trivial WITH full tracing data. Multi-day debugging sessions (BinVoice, WindowsVoiceTranscription) could have been minutes if trace data were visible. David also needs admin-facing audit history so he can look back at client issues WITHOUT involving GSD-T, and for high-compliance projects. Trace and audit are TWO DISTINCT things.
- **TRACE log (default: EVERY project, few exceptions):** expansive capture-everything — data packets, events, transmissions, results, every LLM call, every error/catch, every decision (candidate-vs-chosen). Rule: "there's never enough detail." Toggleable on/off (it's a lot of data; on when debugging or hunting unknown issues). BinVoice `src/trace/` is the proven template: emitter + transport + shared `TraceRecord` DTO (category, decision:boolean|null, detail, optional structured `data` blob); one-line runtime toggle `setTraceEnabled()`; batches POST to server → persisted table; admin-only newest-first viewer; bars PII; governed by a `trace-logging-contract.md`.
- **AUDIT log (default: every project EXCEPT explicit opt-out):** admin-facing persisted history for compliance + client-issue lookback. Distinct from trace. **Does NOT exist in BinVoice** (admin actions only hit ephemeral Pino stdout — no table, no history UI) — so audit must be DESIGNED FRESH: dedicated persisted table + admin history view + per-action schema (who/what/when/before→after). Opt-out example: WindowsVoiceTranscription needs trace, NOT audit.
- **Treatment: CONTRACT + RULE + SCAFFOLDING (David confirmed 2026-07-01).** (1) a logging **contract** defining required trace-entry + audit-entry fields, which `gsd-t-verify` enforces; (2) a CLAUDE.md hard rule making both defaults; (3) `gsd-t-init` scaffolds the trace + audit infrastructure (module + toggle + storage seam) into new projects.
- **KEY REFINEMENT (David 2026-07-01):** the *infrastructure* is baked in uniformly, but the **schema must VARY per project** and be "teased out at the right stage of building the app" — trace categories and audit actions differ between a browser-extension, a transcription app, a compliance-heavy client app. So the contract defines the required SHAPE/envelope (fields every entry must carry) while a per-project step distills the project-specific trace categories + audit actions. For existing projects this distillation must also reconcile whatever ad-hoc logging already exists. Likely a dedicated phase/prompt (during milestone/partition or a `populate`-style pass) that produces the project's concrete category+action list against the universal envelope.
- **Pairs with #47** (build-number) — both framework-default infrastructure; could be one milestone or two.
