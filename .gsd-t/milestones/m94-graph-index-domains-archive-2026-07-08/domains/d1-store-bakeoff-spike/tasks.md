...
- **Dependencies**: none
- **Acceptance criteria**:
  - Generates a synthetic graph of configurable node count (default ~1.5M, the Atos-scale HYPOTHESIS) with realistic file→file import edges + function→function call edges
  ...
  - **[RE-PLAN Fix-6 — scale derived from MEASURED Atos, in graph-node units not raw LOC; NO mid-spike cross-dependency]** the node-count is CONFIGURABLE (a `--nodes`/`--scale` flag). D1 and D2 stay CONCURRENT in Wave 1 (no deadlock): D1's bake-off runs FIRST at the ~1.5M-node HYPOTHESIS (independent, as planned). The scale dimension is GRAPH NODES (files + functions/entities) + EDGES — NOT raw LOC — so K2's `atosFileCount` + entity/edge enumeration (apples-to-apples graph units), not `atosTotalLoc`, is what the synthetic scale is sanity-checked against (`[RULE] k2-scale-sanity-vs-bakeoff`). The reconciliation is a **Wave-1-CLOSE step** (after BOTH spikes report, before the hard gate finalizes K1 PICK): if K2's measured node-equivalent scale materially diverges (>1.5×/<0.66×) from the ~1.5M default, the bake-off is RE-RUN at the corrected `--scale` and K1 PICK is re-confirmed at the right size — never a mid-spike dependency, never a deadlock
...
- **Acceptance criteria**:
  ...
  - **[RE-PLAN Fix-5 — KILL attributable per-candidate per-criterion]** On a `KILL_OR_RESCOPE` verdict, the envelope MUST record, PER CANDIDATE, WHICH sub-criterion failed (e.g. `{candidate: 'jsonl', failed: ['query-latency-over-50ms','incremental-over-1s'], measured: {...}}`) — a bare KILL with no per-candidate per-criterion breakdown is REJECTED (`[RULE] k1-kill-attributable-per-candidate`). The harness ALSO emits a `candidateSetJustification` field: the candidate set is the CLOSURE of the embedded / no-server / no-paid-license category (KuzuDB-embedded · SQLite-recursive-CTE · JSONL · graphology), OR names the next candidate to try BEFORE declaring an architectural kill (a KILL must be attributable to a real ceiling, not an under-sampled candidate list). The T2 killing test asserts: a verdict==KILL_OR_RESCOPE with empty/missing per-candidate per-criterion attribution FAILS; a synthetic all-candidates-fail run MUST record which criterion killed EACH candidate
...
- **Acceptance criteria**:
  ...
  - **[RE-PLAN Fix-4 — machine-checkable Wave-1 gate]** the result-doc envelope MUST carry a structured `k1Verdict` field with value `PICK` or `KILL_OR_RESCOPE` (the verdict the Wave-1 hard-gate test reads) — `[RULE] k1-verdict-field-machine-checkable`. On `KILL_OR_RESCOPE`, the envelope ALSO carries the AC-descope record (which ACs survive) per `[RULE] kill-outcome-records-ac-descope`. The integrate-owned Wave-1 hard-gate test (D7-T2) reads this field; an envelope missing `k1Verdict` FAILS the gate
  - **[RE-PLAN Fix-5 — KILL attributable]** on `KILL_OR_RESCOPE`, the result doc reproduces the per-candidate per-criterion breakdown + `candidateSetJustification` from the T2 envelope (`[RULE] k1-kill-attributable-per-candidate`)
