...
- **Test**: `test/m100-d3-envelope-gate.test.js` — **KILLING TEST (highest-risk piece)**: a battery of synthetic envelopes — (a) valid trace `ts/category/decision/detail` PASSES, (b) valid audit `ts/actor/action/target/before/after/context` PASSES, (c) missing-field FAILS, (d) wrong-type FAILS, (e) PII-shaped value in any trace field (incl. `data`) FAILS, (f) a NOVEL category/action value PASSES (proves structural, never hardcoded), (g) a COLLAPSED single-stream record (trace `decision` marker co-present with audit `before/after` markers) FAILS. If (f) fails or (g) passes, the gate is broken.
  ...
  - **Partial-set collapse killing sub-cases (M100 pre-mortem FINDING 2, HIGH)**: (g4) a record with top-level `category`+`decision` AND `actor`+`action` but NO top-level `before`/`after` → FAILS no-collapse (carries markers of both streams even though the audit set is incomplete); (g5) a record with top-level `decision`+`action` (one marker from each stream) → FAILS.
...
- **ImplPath**: predicate reads the audit module's declared surface + project config; asserts absence of update/delete API in normal operation, presence of a configurable retention setting, and the audit-default-except-opt-out rule. The discovery mechanism this predicate relies on to LOCATE the audit module/store and the opt-out file is pinned in `logging-verify-gate-contract.md` §discovery — see M100-D3-T3 for the per-project `checkLoggingEnvelopes({projectDir})` enumeration built to that section.
- **Acceptance criteria**:
...
- **Test**: `test/m100-d3-envelope-gate.test.js` — asserts the predicate is invoked by the verify gate and that a failing envelope produces a non-zero / blocking result (FAIL-CLOSED, never warn-and-proceed); asserts exactly ONE registration line references `gsd-t-logging-envelope-check.cjs` in `bin/gsd-t-verify-gate.cjs`.
  ...
  - **Discovery killing sub-case (M100 pre-mortem FINDING 1, CRITICAL — VERBATIM required)**: a positive+negative discovery test that runs `checkLoggingEnvelopes({projectDir})` against a REAL fixture project dir — (a) a fixture with a conformant trace module, a conformant audit store, and NO opt-out → PASS; (b) the SAME fixture with one planted broken trace record (missing `category`) → the per-project form must FAIL, proving it actually DISCOVERED and validated that record rather than returning a vacuous pass; (c) a fixture with NO logging artifacts at all and NO opt-out → must FAIL for missing audit (not vacuously PASS).
- **ImplPath**: one `require(...)` + registration entry wiring `checkEnvelope` into the gate's check list; the gate halts on failure per the envelope contracts' Enforcement sections. `checkLoggingEnvelopes({projectDir})` builds REAL enumeration per `logging-verify-gate-contract.md` §discovery — it walks `projectDir` to locate (i) the trace module/store and its records, (ii) the audit module/store and its records, (iii) `.gsd-t/audit-optout.json` — and runs every discovered record through `checkEnvelope`; it is not a stub and never returns `ok:true` on zero-discovery.
- **Acceptance criteria**:
  ...
  - `checkLoggingEnvelopes({projectDir})` discovery is real (per §discovery): fixture with real records + no opt-out → PASS; same fixture with one planted broken record → FAIL; fixture with nothing discoverable + no opt-out → FAIL (never vacuous PASS).
